...
- Wide sibling lists (`old.len() * new.len() > 48`) must uniquely identity-match content-addressed `NodeId`s (singleton on each side) before structural `(kind, payload)` LCS. Several empty-payload `Function` siblings plus an EOF append otherwise shift every LCS pair by one and invent delete/move cascades (see `examples/go-eof-insert-demo/`). Duplicate ids stay for occurrence repair.
