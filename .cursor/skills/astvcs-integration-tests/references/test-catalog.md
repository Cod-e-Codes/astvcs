...
| `struct_field_append_diff_applies_roundtrip` | Appending a struct field diffs and applies to valid Rust (unit, `diff/ast_diff.rs`) |
...
| `complex_struct_field_and_body_disjoint_merge_roundtrip` | Struct field insert + function body edits on separate branches merge to parseable Rust (unit, `merge/mod.rs`) |
| `complex_struct_field_with_println_and_iflet_validate_merge_roundtrip` | Struct field + `println!` / `if let` validate body edits merge to parseable Rust (unit, `merge/mod.rs`) |
...
| `cli_fsck_clean_repository`, `cli_fsck_detects_corruption`, `cli_fsck_repair_fixes_index_inconsistency`, `cli_fsck_repair_refuses_ambiguous_head`, `cli_fsck_repair_leaves_missing_blob`, `cli_fsck_prune_refs_removes_dangling_ref`, `cli_fsck_warns_on_unknown_format_version`, `checkout_rejects_empty_timeline_manifest`, `empty_tree_loads_from_timeline_when_states_dedup_missing` | fsck clean, corruption, repair, prune-refs, unknown format version, corrupt empty manifest rejection, empty-tree timeline fallback |
