...
- Revert conflicts and failed reset on dirty trees leave refs and disk unchanged (see `cli_revert_and_dry_run`, `cli_revert_of_revert_restores_content`, `reset_hard_refuses_dirty_tree_without_force` in `src/store/repo.rs`).
