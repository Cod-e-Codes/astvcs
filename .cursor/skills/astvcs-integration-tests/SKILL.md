...
- Alignment HTML viewer: `cli_diff_view_writes_html_with_alignment` (`diff --view` writes temp HTML with path, intents, and alignment; browser open skipped when `CI` is set). Unit coverage in `diff/view.rs` and `diff/ast_diff.rs` (`diff_graphs_detailed`).
...
- `stash`: `stash_before_checkout`, `stash_pop_restores_files`, `stash_pop_preserves_unstashed_tracked_files`, `stash_pop_conflict_keeps_entry`; unit coverage in `store/stash.rs` (`stash_stack_save_load`, `stash_entry_roundtrip`).
...
- Shallow fetch/clone: `shallow_clone_has_fewer_timeline_entries_than_full_clone`, `merge_base_fails_on_shallow_clone_with_incomplete_history` (integration); `shallow_clone_fetches_fewer_timeline_entries_than_full_clone`, `merge_base_fails_on_shallow_repo_when_lca_missing` (unit, `network/sync.rs`); ancestry API `GET /v1/timeline/{tip}/ancestry?depth=N`.
...
- `import-git`: `import_git_snapshot_from_subprocess` (integration) imports git HEAD via subprocess when `git` is on PATH; unit coverage in `store/git_import.rs` (`parse_ls_tree_line_*`).
