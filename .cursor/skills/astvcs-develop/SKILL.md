...
- If network sync behavior changed, update [docs/architecture.md](../../../docs/architecture.md) network section and extend `tests/integration.rs` or `src/network/` tests (serve auth, HTTPS/TLS, SSH remotes and remote-serve protocol, transport bearer token and `--insecure`, file remote unchanged).
