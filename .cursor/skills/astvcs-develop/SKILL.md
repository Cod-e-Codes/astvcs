...
- If CLI behavior changed (including `--details`, `identity`, `--json`, `reset`, `revert`, `merge`, `rebase`, `cherry-pick`, `blame`, `bisect`, `pull`, `checkout`, `stash`, `tag`, `branch remove`, `branch create`, `default_branch` config sync, `add`, staging index, client hooks (`--no-verify`, `.astvcs/hooks/`), `gc` (`--prune`, `--prune-history`), `fsck` (`--repair`, `--prune-refs`), `repack`, `import-git`, binary file tracking, symlink and executable file modes, materialize dirty-tree guard, remote ref resolution, parse fallback visibility in `status`/`diff`, `diff --view` change-first HTML viewer, incremental scan cache / `--full-scan`, on-disk format versioning / migrations, repository lock errors, shallow fetch/clone `--depth`, serve auth/TLS/concurrency, or SSH/HTTPS remotes), update [docs/commands.md](../../../docs/commands.md) and add or extend a test in `tests/integration.rs`; update [docs/architecture.md](../../../docs/architecture.md) when network, locking, atomicity, reachability, gc/fsck/repack (including two-tier history retention and fsck repair), author identity, structured errors, binary blobs, symlink/mode metadata, parse fallback policy, incremental scan cache, on-disk format versioning, staging index, default branch config, lightweight tags, client hooks, rebase, cherry-pick, blame, bisect, git import interoperability, `diff_graphs_detailed` / alignment export / `diff --view`, or repository model semantics change; refresh [README.md](../../../README.md) (what it is / is not, documentation map, quick start, scope at a glance) when user-facing boundaries or onboarding change.
