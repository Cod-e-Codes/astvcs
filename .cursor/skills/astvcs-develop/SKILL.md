...
| `store/` | blobs, timeline, repo CLI backend, working-tree scan; `lock.rs` (exclusive advisory lock), `atomic.rs` (rename writes) |
...
- If CLI behavior changed (including `reset`, `revert`, `merge`, `checkout`, `branch remove`, materialize dirty-tree guard, remote ref resolution, or repository lock errors), update [docs/commands.md](../../../docs/commands.md) and add or extend a test in `tests/integration.rs`; update [docs/architecture.md](../../../docs/architecture.md) when network, locking, atomicity, or repository model semantics change.
