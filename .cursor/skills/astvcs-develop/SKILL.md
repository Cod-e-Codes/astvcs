...
- If merge, revert, or diff semantics changed, update the matching fixture under `examples/` and its row in [examples/README.md](../../../examples/README.md). After refreshing walkthrough commands, run `.\examples\run-demos.ps1` (or `.\examples\run-demos.ps1 -LogPath <file>`) to replay all nine fixtures non-interactively (workflow, merge, identity, same-file, network, lifecycle, shallow, import-git when `git` is on PATH, serve).
