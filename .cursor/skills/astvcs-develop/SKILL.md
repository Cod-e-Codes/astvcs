...
- If CLI behavior changed (including `reset`, `revert`, `merge`, `checkout`, `branch remove`, materialize dirty-tree guard, or remote ref resolution), update [docs/commands.md](../../../docs/commands.md) and add or extend a test in `tests/integration.rs`; update [docs/architecture.md](../../../docs/architecture.md) when network or repository model semantics change.
