...
**Branches.** Local branch tips live under `.astvcs/refs/heads/`. `branch remove` deletes a ref file only; it refuses the checked-out branch and the last remaining branch. Unmerged commits do not block removal because states are content-addressed and remain in the timeline and blob store until `gc --prune` removes blobs no longer reachable from any ref tip. `config.json` `default_branch` is not updated when a branch is removed.
...
`gc` deletes unreachable blob files only. Timeline entries and state manifests for unreachable states are kept deliberately: astvcs is content-addressed, `branch remove` never deletes history by state id, and timeline entries form a permanent audit log that lets you inspect or recover states by id even after all refs are gone. Pruning timeline entries would break checkout-by-state-id and obscure history with no space benefit comparable to blob payloads. Pass `--prune` to delete unreachable blobs; the default is a dry-run that reports how many blobs would be removed and how much space would be reclaimed.

**Repository integrity (`fsck`).** `fsck` is report-only and never modifies the repository. It checks: state manifests referencing missing blobs; refs pointing to state ids with no timeline entry; HEAD naming a branch with no ref file; `index.json` entries inconsistent with HEAD (wrong `state_id`, paths absent from HEAD manifest, or index present while HEAD is invalid); and orphan `.astvcs-tmp` files whose canonical target does not exist (the cases `cleanup_stray_temp_files` leaves alone). There is no automatic repair mode: ref, HEAD, and index corruption require human judgment about which state to trust, while unreachable blob cleanup belongs to `gc --prune`. Run `fsck` after suspected corruption; fix reported ref or HEAD problems manually, then `gc --prune` if blob orphans remain.
...
| `gc_no_unreachable_is_noop`, `gc_preserves_remote_tracking_blobs`, `gc_twice_is_idempotent` | Reachability GC preserves remote-tracking refs; idempotent prune (unit, `store/integrity.rs`) |
